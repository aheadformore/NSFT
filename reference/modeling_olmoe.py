...
"""Reference OLMoE modeling file with FA2 position_ids fix for packed sequences.

This file is identical to the standard transformers OLMoE modeling code,
with one critical fix: ``position_ids`` is passed to
``_flash_attention_forward`` in the Flash Attention 2 forward method.

This enables block-diagonal attention for packed sequences (where
multiple sub-sequences are concatenated with position_ids resetting
to 0 for each sub-sequence).

Usage:
    1. Copy this file to your model directory, OR
    2. Add the model directory to sys.path and register it with AutoModel:
       sys.path.insert(0, "path/to/this/file's/parent")
...
Without this fix, packing with Flash Attention 2 will produce incorrect
loss (~5.0 instead of ~1.2) due to missing block-diagonal attention.
"""
